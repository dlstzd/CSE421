...
void readNums(int flag, char* filename,int frames,char* repAlgorithm){
  ...
  if(flag == 0){ //stdin
     ...
     if(strcmp(repAlgorithm,"FIFO") == 0){
        ...
        algPageReplacements = FIFO(frames,inBuf);
        stop = stop_time();
     ...
     */
     ...
     printf("# page replacement penalty using %s: %i\n",algPenalty);
     printf("\n");
     printf("\n");
     printf("Total time to run %s algorithm: %f ms\n",repAlgorithm,algTime);
     printf("Total time to run Optimal algorithm: %f ms\n",optimalTime);
     if(algTime < optimalTime){
        printf("%s is %f faster than Optimal algorithm\n",repAlgorithm,percentFaster);
     }else if(algTime > optimalTime){
     ...
     if(strcmp(repAlgorithm,"FIFO")== 0){
        start = start_time();
        fprintf(stderr,"Attempting to call FIFO()\n");
        algPageReplacements = FIFO(frames,buf);
        stop = stop_time();
        algTime = total_time(start,stop);
     }else if(strcmp(repAlgorithm,"LFU") == 0){
        start = start_time();
...
int FIFO(int frames,char *buffer){
